...
class Dungeon(object):
    ...
    def __init__(self, kwargs):
        ...
        self.pre_map2D = [[None for _ in range(self.dungeon_height)]
    ...
    def _add_room(self, rectangle):
        ...
        given in rectangle a floor tile
    ...
    def _build_random_room(self):
        """Get a random room, if it does not intersect other rooms inside the
        dungeon, add it to dungeon rooms"""
        def get_random_rectangle():
            """returns a random rectangle inside the dungeon grid which has
            distance >= 1 from the borders"""
        ...
        # add it to dungeon rooms if it does not intersect existing rooms in
        # the dungeon
        ...
        for m in range(0, self.dungeon_width):
            for n in range(0, self.dungeon_height):
                if self.pre_map2D[m][n] == 'floor':  # if tile is floor
                    ...
                    for c in self.get_neighboring_coordinates((m, n), 1, self_included=False):
                        x, y = c
                        # if there is a dirt tile as its neighbor
                        if self.pre_map2D[x][y] == 'dirt':
                            # make it a wall tile.
                            self.pre_map2D[x][y] = 'wall'

    def _connect_rooms(self):
        """ first connect every room to the closest one, then connect the
        unconnected components"""

        def find_component(room):
            """ return the component if room belongs to one, otherwise return
            False."""
        ...
        def carve_tunnel(x1, y1, x2, y2):
            ...
            tunnel_x = [(m, y1) for m in range(x1, x2)]
            tunnel_y = [(x2, n) for n in range(min(y1, y2), max(y1, y2) + 1)]
            tunnel = tunnel_x + tunnel_y
            for x, y in tunnel:
                self.pre_map2D[x][y] = 'floor'

        # initially all rooms are unconnected
        unconnected_rooms = list(self.rooms)
        ...
        for room1 in unconnected_rooms:
            ...
            # if room1, room2 do not belong to any component
            if not comp1 and not comp2:
                # put them into same component
                connected_components.append({room1, room2})
            # if room1 belongs to a component but not room2
            elif comp1 and not comp2:
                comp1.add(room2)  # put room2 into the component of room1
            elif not comp1 and comp2:  # similar
                comp2.add(room1)
            # if both rooms belong to different components
            elif comp1 and comp2 and comp1 != comp2:
                # remove the second component
                connected_components.remove(comp2)
        ...
        if len(connected_components) > 1:  # there are unconnected components
            ...
            for component in connected_components:
                # pick a random room
                still_unconnected_rooms.append(choice(list(component)))

            # for each room in still unconnected rooms
            for j, room in enumerate(still_unconnected_rooms):
                # except the last one
                if j != len(still_unconnected_rooms) - 1:
                    other = still_unconnected_rooms[j + 1]
                    x1, y1, x2, y2 = room.x, room.y, other.x, other.y
                    # connect it with the next room in the unconnected rooms
                    # list
                    carve_tunnel(x1, y1, x2, y2)
    ...
    def _set_map_from_pre_map(self):
        ...
        for m in range(0, self.dungeon_width):
            for n in range(0, self.dungeon_height):
                self.map2D[m][n] = Tile(
                    coordinates=(m, n), tip=self.pre_map2D[m][n])

    def get_neighboring_coordinates(self, center, radius=1, self_included=True):
        """Returns coordinates which are neighbors of given x, y and lies in
        the dungeon grid"""
        coordinates = []
        x, y = center
    ...
    def get_neighboring_tiles(self, tile, radius=1, self_included=True):
        """Returns tiles which are in a neighborhood of given radius and given
        tile and lies in the dungeon grid
        :rtype : List of Tile Objects
        :param tile: Tile
        :param radius: Integer
        :param self_included: Boolean,
        """
        neighboring_coordinates = self.get_neighboring_coordinates(
            tile.coordinates, radius, self_included)
